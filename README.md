...
Eigen differential tests live in the non-published `tools/eigen-harness` package. The core crate has no Eigen feature, C++ build script, or `cc` build dependency. Repository CI installs Eigen only for that harness; ordinary default/`std` consumers never perform Eigen discovery or compile the bridge.
...
The repository keeps four executable teaching examples: [streaming square-root RLS](examples/recursive_least_squares.rs), [batch QR fitting from a borrowed buffer](examples/mapped_least_squares.rs), a [two-state position/velocity Kalman filter](examples/kalman_1d.rs), and [embedded storage budgeting](examples/embedded_resource_budget.rs). Build and documentation CI execute these examples. They demonstrate library usage, not full navigation systems or representative performance baselines. Start with [online calibration using RLS](docs/tutorial-recursive-least-squares.md); its estimator retains no sample history and is compiled through a separate `no_std` Cortex-M consumer. For model assumptions and other guidance, see [Tutorials](docs/tutorials.md), [Getting started](docs/getting-started.md), [API usage](docs/api-usage.md), and [Use cases](docs/use-cases.md).
