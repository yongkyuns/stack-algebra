...
// The host driver reuses the companion's exact synthetic fixture, not its solver.
// The standalone estimator module above has no dependency on this data or std.
#[allow(dead_code)]
#[path = "mapped_least_squares.rs"]
mod batch;
...
fn main() -> std::io::Result<()> {
    ...
    // ANCHOR: initialize
    let mut estimator = QuadraticRls::<f64>::new(
        INPUT_SCALE,
        PRIOR_PRECISION,
        FORGETTING,
        Matrix::zeros(),
    ...
    // ANCHOR: stream
    for step in 0..batch::SAMPLE_COUNT {
        // A fixed permutation covers the input range early; it changes no (x,y) pair.
        let source_row = (17 * step) % batch::SAMPLE_COUNT;
        let x = batch::sample_x(source_row);
        let y = batch::reference_y(x) + batch::NOISE_SCALE * batch::NOISE[source_row];
    ...
    if curve_csv {
        ...
        for (step, snapshot) in &snapshots {
            ...
            for row in 0..batch::CURVE_COUNT {
                let x = batch::curve_x(row);
                ...
                tutorial_output::csv_row(&[
                    *step as f64, x, fitted_y, batch::reference_y(x),
                    theta[(0, 0)], theta[(1, 0)], theta[(2, 0)],
                    (snapshots.len() * batch::CURVE_COUNT) as f64,
                ]);
            }
        }
    ...
    if csv {
        ...
        println!("samples = {}, forgetting = {:.3}, prior precision = {:.6}", history.len(), FORGETTING, PRIOR_PRECISION);
    ...
    for (index, (source_row, x, y, update)) in history.iter().enumerate() {
        ...
        if csv {
            tutorial_output::csv_row(&[
                (index + 1) as f64, *source_row as f64, *x, *y, batch::reference_y(*x),
                update.prediction_before, update.innovation,
                theta[(0, 0)], theta[(1, 0)], theta[(2, 0)], fitted, *y - fitted,
                batch::REFERENCE_COEFFICIENTS[0], batch::REFERENCE_COEFFICIENTS[1],
                batch::REFERENCE_COEFFICIENTS[2], INPUT_SCALE, PRIOR_PRECISION, FORGETTING,
                history.len() as f64, bytes32 as f64, bytes64 as f64,
            ]);
        } else {
            println!("{} {:.3} {:.3} {:.3} {:.3} {:.3} {:.3} {:.3}", index + 1, x, y,
                update.prediction_before, update.innovation,
                theta[(0, 0)], theta[(1, 0)], theta[(2, 0)]);
        }
    }
