...
macro_rules! scalar_benches {
    ($module:ident, $scalar:ty, $scalar_name:literal, $tolerance:expr) => {
        mod $module {
            ...
            fn faer_rhs<const D: usize, const P: usize>(rhs: &Matrix<D, P, $scalar>) -> Mat<$scalar> {
                Mat::from_fn(D, P, |row, column| rhs[(row, column)])
            }

            fn stack_from_faer<const D: usize, const P: usize>(matrix: &Mat<$scalar>) -> Matrix<D, P, $scalar> {
            ...
            fn bench_cholesky<const D: usize, const P: usize>(criterion: &mut Criterion) {
                ...
                let mut group = criterion.benchmark_group(concat!("dense-multi-rhs/llt/", $scalar_name));
                let shape = format!("{D}x{P}");
                group.bench_with_input(BenchmarkId::new("stack-algebra", &shape), &(), |bench, _| {
                    bench.iter(|| {
            ...
            fn bench_ldlt<const D: usize, const P: usize>(criterion: &mut Criterion) {
                ...
                let mut group = criterion.benchmark_group(concat!("dense-multi-rhs/ldlt/", $scalar_name));
                let shape = format!("{D}x{P}");
                group.bench_with_input(BenchmarkId::new("stack-algebra", &shape), &(), |bench, _| {
                    bench.iter(|| {
                        black_box(&stack_factor)
            ...
            fn bench_lu<const D: usize, const P: usize>(criterion: &mut Criterion) {
                ...
                let mut group = criterion.benchmark_group(concat!("dense-multi-rhs/lu/", $scalar_name));
                let shape = format!("{D}x{P}");
                group.bench_with_input(BenchmarkId::new("stack-algebra", &shape), &(), |bench, _| {
                    bench.iter(|| {
                        black_box(&stack_factor)
            ...
            fn bench_triangular<const D: usize, const P: usize>(criterion: &mut Criterion) {
                ...
                let mut group = criterion.benchmark_group(concat!("dense-multi-rhs/triangular/", $scalar_name));
                let shape = format!("{D}x{P}");
